...
class Trainer:
    def __init__(self, model : GaussianModel, dataset : Dataset,
                 settings : Settings, server_controller = None):
        self.model : GaussianModel = model
        self.dataset : Dataset = dataset
        self.settings = settings
        self.initialized = False
        self.average_train_step_ms = 0.0
    ...
    async def handle_train_settings(self, data, websocket):
        ...
        for k in data.keys():
            ...
            if k not in self.settings.keys():
                await self.server_controller.send_error_message("Training settings error", 
                        f"Key {k} from client not present in Settings")
                return
        ...
        try:
            ...
            await self.server_controller.send_error_message("Training settings error", 
                    f"Failed to update trainer settings.")
            return
